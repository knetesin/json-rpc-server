...
`Started` всегда первый. Затем ровно одно из `Completed` или `Failed`.
...
- одна строка на каждый вызов в request'е,
- имя метода, класс handler'а, нормализованные params,
- статус (`ok` / `error`), длительность, флаг cache-hit,
- preview результата (или исключение + message на ошибках).
